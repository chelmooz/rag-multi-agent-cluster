...
> ℹ️ **Beta test frontend** : voir `scripts/smoke_test_frontend_api.py` — validation automatisée API + frontend (35 scénarios, **35/35 PASSED**). **`/api/embed`, `/api/query`, `/api/ingest` sont implémentés** (Phase A) mais renvoient `503` tant que les services (Ollama/Qdrant) ne sont pas démarrés en dehors du lifespan applicatif. **Couverture unitaire réelle** : `ingestion.py` 98 %, `vector.py` 100 %, `lexical.py` 95 %, `reranker.py` 100 %, `ollama.py` 97 %, couverture globale `src/` 83 % (410/410 tests, ruff 0, mypy 0). Seuls `/lint` et `/okf/*` restent de vrais stubs `NotImplementedError` (ROADMAP Phase B).
