...
- [x] **§5.13 Revue de code Clean Code / KISS / SOLID** — **04/08/2026** : audit 43 fichier sourcess. Score 8.5/10. Refactorings appliqués : (1) `ollama.py` — `vision()` utilisait `_request()` privé → extraction méthode publique `generate_with_images()` (SOLID violation corrigée) ; (2) `wiki_agent.py` — `lint()` orphelins O(n²) reads → lecture unique cache (KISS optimisation) . ruff/mypy 0, 410/410 tests passent (Windows excludes temp-path fixtures).
- [x] **§5.14 Correction dérive documentation couverture** — **04/08/2026** : README.md ligne 13 affirmait couverture ~30% pour `ingestion.py`/`vector.py`/`lexical.py`/`reranker.py`/`ollama.py` — corrigé avec valeurs réelles (98%/100%/95%/100%/97%, couverture globale src/ 83%). ROADMAP.md ligne A8.1-A8.4 : mêmes valeurs de couverture obsolètes (27%/26%/31%/33%/30% du 01/08 audit) → mises à jour ✅ avec couverture réelle. Note ajoutée : tests hybrid_search répartis dans modules existants (DRY), pas de `test_hybrid_search.py` isolé.
- [x] **§5.15 Audit pré-déploiement externe (04/08/2026) — note 6/10 NOGO, 2 bloqueurs corrigés** :
  - **Bloqueur 1 — crash-loop Docker C1 (corrigé)** : `Dockerfile.langgraph` (CMD `python -m src.agents.langgraph_orchestrator`) et `Dockerfile.wiki-agent` (CMD `python -m src.agents.wiki_agent`) exécutaient `main()` en une passe puis `return` → `restart: unless-stopped` = reboot infini. Fix : `langgraph_orchestrator.py` — `run_worker()` consommateur Redis BRPOP `rag:pipeline:queue` (timeout 5s, résilient aux jobs en échec : `ok:false` + `error`, LPUSH résultat `rag:pipeline:results` en JSON), `build_pipeline_services()` miroir de `rag.py` (pool/vector/lexical/reranker/agents), `_job_result()` réduit `PipelineState` en payload JSON-safe ; `wiki_agent.py` — `run_service()` boucle maintenance (update_index + lint + append_log trace), intervalle env `WIKI_MAINTENANCE_INTERVAL` (défaut 3600s). Arrêt propre KeyboardInterrupt (SIGTERM Docker). CMD Dockerfiles inchangés (les `main()` bouclent désormais).
